...
void NetworkDef::locate()
{
  ...
  std::ofstream state_machine_file;
          ...
          for (uint16_t i = 0; i < n->m_numAttribDataHandles; ++i)
          {
              if (n->m_nodeAttribDataHandles[i].m_attribData)
              {
                  ...
                  else if (type == ATTRIB_TYPE_STATE_MACHINE_DEF)
                  {
                      ...
                      auto output_list = [&state_machine_file, &a](uint16_t* ids, uint16_t num, uint16_t index) {
                      ...
                      auto output_state_def = [&state_machine_file, output_list, &a](StateDef* sd, std::string& name_or_id, uint16_t childid) {
